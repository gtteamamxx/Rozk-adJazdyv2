...
namespace RozkładJazdyv2.Model
{
    public class TimetableDownloadService : Timetable
    {
        ...
        private static void ResetIdentificators()
        {
        ...
        private static async ValueTask<Line> GetLineDetailInfoAsync(Line line)
        {
            ...
            using (var lineAngSharpDocument = (await GetAngleSharpDocumentOfSiteAsync(url)))
            {
                ...
                IElement lineHtmlElement = lineAngSharpDocument.Children.Count() >= 1 
                    ? lineAngSharpDocument.Children[0] : null;
        ...
        private static async ValueTask<Track> GetTrackDetailFromHtmlAsync(IElement htmlTrack, Track track)
        {
            ...
            string trackUrl = $"{_TIMETABLE_BASE_URL}{linkToFirstStop}";
            track.Url = trackUrl;
        ...
        private static async ValueTask<BusStop> GetStopHoursAsync(Track track, BusStop busStop)
        {
            using (var htmlDocument = await GetStopAngleSharpDocumentAsync(busStop))
            {
                ...
                IEnumerable<IElement> listOfHtmlHours = htmlDocument.QuerySelectorAll("table")
        ...
        {
            IEnumerable<IElement> listOfHtmlHours = htmlDayType.QuerySelectorAll("span")
        ...
        private static void GetSchedulesDetail(ref Line line, IEnumerable<IElement> listOfHtmlSchedules)
        {
            ...
            var rawHtmlSchedules = listOfHtmlSchedules.ElementAt(0).Children[1].QuerySelectorAll("li a");

            foreach (var rawHtmlSchedule in rawHtmlSchedules)
            {
                listOfSchedules.Add(new Schedule()
                {
                    Id = _ScheduleId++,
                    IdOfLine = line.Id,
                    IsActualSchedule = rawHtmlSchedule.TextContent.Contains("obecnie"),
                    Name = rawHtmlSchedule.TextContent,
                    Url = string.Format("{0}{1}", _TIMETABLE_BASE_URL, rawHtmlSchedule.GetAttribute("href"))
                });
            }
